...
for yy in range(beg_y,end_y+1):
 ...
 for vv in nc_file.veget.data:
   FCOS=factor_vcos(LRU_case,vv+1,yy,nc_file.sel(veget=vv)) 
   fcos[:,vv,:,:]= np.copy(FCOS)*MS/MC  #*10**(6)
    
 fcos=np.squeeze(np.sum(fcos,axis=1))
 ds = xr.Dataset({'flx_cos': (['time','lat', 'lon'], fcos)},
                            coords={'lat': (['lat'], nc_file.lat.values),
                                    'lon': (['lon'], nc_file.lon.values),
                                    'time': (['time'],nc_file.time.values)})

 ds["flx_cos"].values*=(-1)
 ds.to_netcdf(dir_flux+'/flx_cos_da_'+str(yy)+'_LRU'+str(LRU_case)+'_dyn.nc', mode='w')
 ds_out=lmdz2pyvar(ds,'flx_cos')
 ds_out=ds_out.get(['flx_cos'])
 ds_out.attrs["units"] = "kgC/m2/s"
 ds_out.to_netcdf(dir_flux+'/flx_cos_da_'+str(yy)+'_LRU'+str(LRU_case)+'_phy.nc', mode='w')
